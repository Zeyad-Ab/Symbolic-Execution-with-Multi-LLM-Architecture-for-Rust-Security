...
def generate_comprehensive_explanations(results: Dict[str, Dict[str, Any]]) -> None:
    """Generate comprehensive explanation files for all results"""
    print("\nGenerating comprehensive vulnerability explanations...")
    ...
    for file_path, result in results.items():
        ...
        explanation_file = file_path.replace('.rs', '_comprehensive_analysis.txt')
        
        try:
            with open(explanation_file, 'w') as f:
                f.write(f"COMPREHENSIVE VULNERABILITY ANALYSIS REPORT\n")
                f.write(f"File: {file_path}\n")
                f.write(f"Dataset Type: {dataset_type.upper()}\n")
                f.write(f"Analysis Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"Total Vulnerabilities: {len(result['vulnerabilities'])}\n")
                f.write(f"Analysis Coverage: {result['coverage']:.2%}\n")
                f.write("="*60 + "\n\n")
                ...
                if result['vulnerabilities']:
                    for i, vuln in enumerate(result['vulnerabilities'], 1):
                        ...
                        f.write(f"Analysis Phase: {vuln.get('phase', 'Unknown')}\n")
                        f.write(f"Confidence: {vuln.get('confidence', 0.0):.2f}\n")
                        
                        # Add comprehensive explanation
                        explanation = get_comprehensive_vulnerability_explanation(
                            vuln.get('type', ''), dataset_type
                        )
                    ...
                    if dataset_type == "positive":
                        f.write("WARNING: This positive file should contain vulnerabilities!\n")
                        f.write("This may indicate a false negative in the analysis.\n")
                    else:
                        f.write("SUCCESS: This negative file is clean as expected.\n")
                    f.write("-" * 40 + "\n\n")
            
            explanation_count += 1
            print(f"  Generated: {Path(explanation_file).name}")
            
        except Exception as e:
            print(f"  Failed to generate explanation for {file_path}: {e}")
    
    print(f"SUCCESS: Generated {explanation_count} comprehensive explanation files")

def get_comprehensive_vulnerability_explanation(vuln_type: str, dataset_type: str) -> str:
    """Get comprehensive explanation for vulnerability types"""
    base_explanations = {
        "buffer_overflow": """
BUFFER OVERFLOW VULNERABILITY:
A buffer overflow occurs when a program writes more data to a buffer than it can hold.
This can overwrite adjacent memory locations, potentially leading to:
- Program crashes
- Data corruption  
- Code execution (if overwriting return addresses)
- Security vulnerabilities

PREVENTION:
- Use bounds checking before array access
- Use safe Rust APIs (Vec<T>, String, etc.) instead of raw pointers
...
- Use reference counting (Rc, Arc) carefully
- Set pointers to null after freeing
- Use RAII patterns
""",
        "unsafe_operation": """
UNSAFE OPERATION VULNERABILITY:
Unsafe operations bypass Rust's memory safety guarantees.
While sometimes necessary, they can lead to:
...
- Use safe wrappers when possible
- Thoroughly test unsafe code
- Implement proper validation
"""
    }
    
    base_explanation = base_explanations.get(vuln_type, """
UNKNOWN VULNERABILITY TYPE:
This vulnerability type requires further investigation.
Please review the code carefully and consider:
...
""")
    ...
    if dataset_type == "positive":
        context = f"""
DATASET CONTEXT - POSITIVE FILE:
This file is from the positive dataset and is expected to contain vulnerabilities.
The detected {vuln_type} vulnerability confirms the file's classification as vulnerable.
This is a real-world example of how {vuln_type} vulnerabilities can occur in production code.
"""
    else:
        context = f"""
DATASET CONTEXT - NEGATIVE FILE:
This file is from the negative dataset and should be clean.
The detection of {vuln_type} vulnerability may indicate:
- A false positive in the analysis
- An actual vulnerability that needs investigation
- A misclassification in the dataset
"""
    
    return base_explanation + context
...
def analyze_comprehensive_dataset(positive_dir: str = "Positive", negative_dir: str = "Negative") -> Dict[str, Any]:
    ...
    generate_comprehensive_explanations(results)
